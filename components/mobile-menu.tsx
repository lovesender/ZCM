    ...
    <div className="md:hidden">
        ...
        <div className="w-6 h-6 flex flex-col justify-center items-center">
          <span
            className={`block w-5 h-0.5 bg-current mb-1 transition-all duration-300 ${
              mounted && isOpen ? "rotate-45 translate-y-1.5" : ""
            }`}
          />
          <span
            className={`block w-5 h-0.5 bg-current mb-1 transition-all duration-300 ${
              mounted && isOpen ? "opacity-0" : ""
            }`}
          />
          <span
      ...
      {mounted && isOpen && (
        <div className="fixed inset-0 z-50">
          {/* 배경 오버레이 */}
          <div className="fixed inset-0 bg-black bg-opacity-50" onClick={closeMenu} aria-hidden="true" />
              ...
              <div className="flex items-center justify-between p-4 border-b border-gray-200">
                ...
                >
                  <div className="w-6 h-6 flex items-center justify-center relative">
                    <span className="block w-4 h-0.5 bg-current rotate-45 absolute" />
                    <span className="block w-4 h-0.5 bg-current -rotate-45 absolute" />
                  </div>
                </button>
              </div>
